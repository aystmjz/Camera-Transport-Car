...
void MOTOR_X(int32_t x, Camera camera, int32_t spead)
{
    ...
    if (x >= 0) {
        if (x >= MOTOR_SlowLong) {
            while ((MOTOR_CONTROL(spead / 2 + E(camera) - DeltaAngleToSpead(), x, MOTOR_Left) > (x - MOTOR_SlowLong)) & (MOTOR_CONTROL(spead / 2 + E_(camera) + DeltaAngleToSpead(), x, MOTOR_Right) > (x - MOTOR_SlowLong))) { /*Display();*/
            }
            while (MOTOR_CONTROL(spead + E(camera) - DeltaAngleToSpead(), x, MOTOR_Left) > MOTOR_SlowLong & MOTOR_CONTROL(spead + E_(camera) + DeltaAngleToSpead(), x, MOTOR_Right) > MOTOR_SlowLong) { /*Display();*/
            }
            ...
            while (MOTOR_CONTROL(spead + E(camera) - DeltaAngleToSpead(), x, MOTOR_Left) & MOTOR_CONTROL(spead + E_(camera) + DeltaAngleToSpead(), x, MOTOR_Right)) { /*Display();*/
            }
        }

    } else {
        if (x <= -MOTOR_SlowLong) {
            while ((MOTOR_CONTROL(spead / 2 + E_(camera) + DeltaAngleToSpead(), x, MOTOR_Left) > (-x - MOTOR_SlowLong)) & (MOTOR_CONTROL(spead / 2 + E(camera) - DeltaAngleToSpead(), x, MOTOR_Right) > (-x - MOTOR_SlowLong))) { /*Display();*/
            }
            while (MOTOR_CONTROL(spead + E_(camera) + DeltaAngleToSpead(), x, MOTOR_Left) > MOTOR_SlowLong & MOTOR_CONTROL(spead + E(camera) - DeltaAngleToSpead(), x, MOTOR_Right) > MOTOR_SlowLong) { /* Display();*/
            }
            while (MOTOR_CONTROL(spead / 2 + E_(camera) + DeltaAngleToSpead(), x, MOTOR_Left) & MOTOR_CONTROL(spead / 2 + E(camera) - DeltaAngleToSpead(), x, MOTOR_Right)) { /* Display();*/
            }
        } else {
            while (MOTOR_CONTROL(spead + E_(camera) + DeltaAngleToSpead(), x, MOTOR_Left) & MOTOR_CONTROL(spead + E(camera) - DeltaAngleToSpead(), x, MOTOR_Right)) { /* Display();*/
            }
        }
    }
    MOTOR_Clear(MOTOR_Left | MOTOR_Right);
...
void MOTOR_F(int32_t x, int32_t spead)
{
    MOTOR_Clear(MOTOR_Left | MOTOR_Right);
    if (x >= 0) {
        while (MOTOR_CONTROL(spead - DeltaAngleToSpead(), x, MOTOR_Left) & MOTOR_CONTROL(spead + DeltaAngleToSpead(), x, MOTOR_Right)) { /*Display();*/
        }
    } else {
        while (MOTOR_CONTROL(spead + DeltaAngleToSpead(), x, MOTOR_Left) & MOTOR_CONTROL(spead - DeltaAngleToSpead(), x, MOTOR_Right)) { /* Display();*/
        }
...
int main(void)
{
    ...
    switch (-1) {
        ...
        while (1)
        {
            ...
            Hook_All_Down();
            return 0;
            MOTOR_X(2000, BEHIND, Spead);
            return 0;
            while (1) {
                Put_ThreeBlock(Spead);
                return 0;
                Hook_All_Down();
                ...
                Delay_ms(1000);
                MOTOR_X(700, FRONT, Spead);
                Hook_All_Down();
                MOTOR_Adjust_X(6250, FRONT, BackCenter, BackSpead);
                // MOTOR_X(1800, FRONT,Spead);
                return 0;
            }
        ...
        case -1:
            while (1) {
                //     Delay_ms(2000);
                //     MOTOR_Spin(Right, 45, 40);
                //     MOTOR_Spin(Left, 45, 40);
                // }
                while (1) {

                    // MOTOR_X(4250, FRONT, 50);
                    // MOTOR_X(2500, FRONT, Spead);
                    MOTOR_X(6200, FRONT, 50);
                    Claw(0);
                    Platform(20);
                    MOTOR_Spin(Left, 180, Spead, A_Spead);
                    ...
                    MOTOR_PutSpin(4, Spead, A_Spead - 10);
                    MOTOR_X(-4100, BEHIND, Spead);
                    Save_ColorData();
                    Hook_All_Down();
                    Delay_ms(500);
